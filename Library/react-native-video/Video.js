...
export default class Video extends Component {
  ...
  render() {
    ...
    let uri = source.uri;
    const isAbsolute = !!source.isAbsolute;
    if (!isAbsolute && uri && uri.match(/^\//)) {
      uri = `file://${uri}`;
    }

    const isNetwork = !!(uri && uri.match(/^https?:/)) && !isAbsolute;
    const isAsset = !!(uri && uri.match(/^(assets-library|file|content):/)) && !isAbsolute;
