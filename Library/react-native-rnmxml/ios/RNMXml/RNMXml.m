...
RCT_EXPORT_METHOD(queryXml:(NSString*)string queries:(NSArray*)queries
                  ...
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  ...
  if (err) {
    reject(@"500", [NSString stringWithFormat:@"%@: Error during parsing.", ERR_DOMAIN], err);
  }
  else {
    resolve(results);
  }
...
RCT_EXPORT_METHOD(queryHtml:(NSString*)string queries:(NSArray*)queries
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  NSError* err;
...
RCT_EXPORT_METHOD(parseString:(NSString*)string isHtml:(BOOL)isHtml
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  NSError* err;
